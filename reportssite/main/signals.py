...
from .models import ChangeLogs, ACTION_CREATE, ACTION_UPDATE, ACTION_DELETE
from .my_validators import ChangeloggableMixin

# сигналы для записи действий пользователей
...
def journal_save_handler(sender, instance, created, **kwargs):
  if isinstance(instance, ChangeloggableMixin):
    ...
    if changed:
      ...
      if created:
        ChangeLogs.add(instance, loggedIn.current_user, loggedIn.address, ACTION_CREATE, changed, id=last_saved['id'])
      else:
        ChangeLogs.add(instance, loggedIn.current_user, loggedIn.address, ACTION_UPDATE, changed, id=last_saved['id'])
...
def journal_delete_handler(sender, instance, using, **kwargs):
  if isinstance(instance, ChangeloggableMixin):
    loggedIn = LoggedInUser()
    last_saved = get_last_saved(loggedIn.request, instance)
    ChangeLogs.add(instance, loggedIn.current_user, loggedIn.address, ACTION_DELETE, {}, id=last_saved['id'])
