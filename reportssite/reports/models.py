...
class Reports(ChangeloggableMixin, models.Model):
    ...
    def __str__(self):
        ...
        return f'{self.service_center.title} за {MONTH_CHOICES [date.month-1][1]}{date.strftime(" %Y")} года'
...
class ReportsRecords(ChangeloggableMixin, models.Model):
    ...
    client_type = models.CharField(choices=ClientType.choices, max_length=50, null=True, blank=True, verbose_name='Тип клиента')
    ...
    client_email = models.EmailField(max_length=500, null=True, blank=True, verbose_name='E-mail клиента')
    ...
    end_date = models.DateField( null=True, blank=True, verbose_name='Дата окончания ремонта')
    ...
    order_parts = models.BooleanField(default=False, verbose_name='Запчасти заказаны')
...
class ReportsParts(ChangeloggableMixin, models.Model):
    ...
    document = models.CharField(null=True, blank=True, max_length=250, verbose_name='Реквизиты накладной')
    record = models.ForeignKey(ReportsRecords, verbose_name='Отчет', on_delete=models.CASCADE)
    order_date = models.DateField(null=True, verbose_name='Дата заказа', blank=True)
    send_date = models.DateField(null=True, verbose_name='Дата отправки', blank=True)
    send_number = models.CharField(null=True, max_length=250, verbose_name='Номер РПО', blank=True)
...
class RecordMembers(models.Model):
    ...
    record = models.ForeignKey(ReportsRecords, on_delete=models.CASCADE, verbose_name='Запись')
...
def part_post_delete(sender, instance, **kwargs):
    ...
    if not instance.order_date:
        parts = instance.record.reportsparts_set.filter(order_date__isnull = True)
        parts_cost = 0
        for one_part in parts:
            parts_cost += one_part.count * one_part.price
        instance.record.parts_cost = parts_cost
        instance.record.total_cost = instance.record.parts_cost + instance.record.move_cost + instance.record.work_cost            
        instance.record.save()
    else:
        parts = instance.record.reportsparts_set.filter(order_date__isnull = False)
