...
from django.views.generic.edit import CreateView, DeleteView, ProcessFormView, FormView, UpdateView
...
class ActFormMixin(FormView):
    ...
    def form_valid(self, form):
        ...
        if 'submit-send' in self.request.POST:
            ...
            if send_errors:
                ...
                return self.form_invalid(form)
            # изменения статуса
            if (form.instance.status.status
                    in [models.ActStatus.DRAFT, models.ActStatus.REFINEMENT]):
                if form.instance.status.status == models.ActStatus.DRAFT:
                    form.instance.add_status(models.ActStatus.SEND, self.request.user)
                else:
                    form.instance.add_status(models.ActStatus.SEND_AGAIN, self.request.user)
                ...
                message = None
                Thread(target=mail_message_for_staff, args=(
    ...
    def get_context_data(self, **kwargs):
        ...
        context['title'] = (
            ...
            if self.object else 'Новый акт неремонтопригодности'
            )
        context['exist_files'] = (
...
class GetCodeView(LoginRequiredMixin, ProcessFormView):
    def get(self, request):
        if 'product' in request.GET and request.GET['product']:
            ...
            select = int(request.GET.get('select')) if request.GET.get('select') else None
...
class ActDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['title'] = f'Акт №{self.object.pk} от {self.object.doc_date.strftime("%d.%m.%Y")}.'
        return context
...
class ActStaffChangeStatusView(StaffUserMixin, ProcessFormView):
    def post(self, request):
        if 'act_pk' in request.POST and request.POST['act_pk']:
            obj = get_object_or_404(models.NonRepairabilityAct, pk=request.POST['act_pk'])
            ...
            Thread(target=mail_message_for_center, args=(
                obj.center,
                subject,
                subject,
                message
...
class ActDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def form_valid(self, form):
        if self.request.user.is_staff:
            obj = self.get_object()
            subject = (
                f'Акт НРП №{obj.pk} от {obj.doc_date.strftime("%Y-%m-%d")} удален.'
                )
            message = self.request.POST['message'] if 'message' in self.request.POST else None
            Thread(target=mail_message_for_center, args=(
                obj.center,
                subject,
                subject,
...
class ActListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if not self.request.user.is_staff:
            ...
            id = [act.id for act in qset if act.status.status != models.ActStatus.DRAFT]
        ...
        if self.request.GET.get("staff_user", '') != '':
            qset = qset.filter(service_center__staff_user=self.request.GET.get("staff_user"))
        if 'status' in self.request.GET and self.request.GET.get("status", '') != '':
            id = [act.id for act in qset if act.status.status == self.request.GET.get("status")]
            qset = qset.filter(id__in=id)
        if ('year' in self.request.GET and 'month' in self.request.GET
            ...
            qset = qset.filter(doc_date__gte=month_start, doc_date__lte=month_end)
        return qset
...
def download_pdf(request, pk):
    ...
    if request.user.is_staff or request.user == act.center.user:
        ...
        response['Content-Disposition'] = f'attachment; filename=renova_nrp_{pk}.pdf'
