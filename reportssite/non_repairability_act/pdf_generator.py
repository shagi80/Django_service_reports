...
def prepare_pdf(act):
    ...
    title_table = Table(
        [
            ...
            [Paragraph(f'{act.pk} от {act.doc_date.strftime("%d.%m.%Y")}', styleTitle), ''],
            ['', ''],
            [Paragraph(f'{act.status.get_status_display()}', styleTitle),''],
            [Paragraph(f'{act.status.created_at.strftime("%d.%m.%Y")}', styleTitle),''],
            ['','']
        ],
    ...
    if act.receipt_date:
        add_row('Дата поступления в ремонт:', act.receipt_date.strftime("%d %m %Y"))
    ...
    if act.device_location:
        elements.append(
            Paragraph(
                f'Изделие {act.get_device_location_display()}.',
                styleTitle
            ))
    doc.build(elements)
