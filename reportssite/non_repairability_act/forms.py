...
class ActEditForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'client': forms.TextInput(
                attrs={'class': 'form-control form-control-sm', 'placeholder': 'ФИО'}
                ),
            'client_phone': forms.TextInput(
                attrs={'class': 'form-control form-control-sm', 'placeholder': 'Телефон'}
                ),
            'client_addr': forms.TextInput(
                attrs={'class': 'form-control form-control-sm', 'placeholder': 'Адрес'}
                ),
            'client_email': forms.EmailInput(
                attrs={'class': 'form-control form-control-sm', 'placeholder': 'E-mail'}
                ),
            'shop': forms.TextInput(
                attrs={'class': 'form-control form-control-sm', 'placeholder': 'Наименование'}
                ),
    ...
    def __init__(self, user = None, disabled = None, *args, **kwargs):
        ...
        if not self.instance.pk:
            self.fields['center'].queryset = ServiceCenters.objects.filter(user=user)
        ...
        if 'product' in self.data and self.data.get('product'):
            ...
            self.fields['code'].queryset = self.instance.product.code_product.all()
...
class ActsFilterForm(forms.Form):
    ...
    status = forms.ChoiceField(
        choices=[('', 'все статусы ...'),] + models.ActStatus.choices,
        required=False,
        widget=forms.Select(
            attrs={"class": "form-select form-select-sm"}
            )
